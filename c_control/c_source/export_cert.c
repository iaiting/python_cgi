...
int export_cert(char *cert)
{
    ...
    // 在设备里的ECCTestApp应用下创建importcertcontainer容器

    rv = SKF_CreateContainer(hApp1, "importcertcontainer", &hContainer1);
	if (rv == 167772215){
		printf("open container\n");
		rv = SKF_OpenContainer(hApp1, "importcertcontainer", &hContainer1);	
...
*/
	...
	rv = SKF_ExportCertificate(hContainer1, bSignflag, cert, &ulCerTextLen);
	if (rv != SAR_OK)
	{
