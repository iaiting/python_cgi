...
#define DEF_DEVICE_PATH "/dev/sda1"
...
int update_firmware(char *szFirmWarePath);
...
int test_update_firmware()
{
    ...
    memset(szFirmWarePath,0x00,260);
    printf("\nupdate firmware....\n");
	...
	update_firmware(szFirmWarePath);
...
int verify_file_system()
{
	...
	{
		printf("#### sn ( %s ) changed after update firmware....\n",stTempDevInfo.SerialNumber);
		return 1;
	}
#endif
#else
